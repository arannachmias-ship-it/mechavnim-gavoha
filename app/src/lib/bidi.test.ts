...
describe("isolateMath – מה אסור לבודד (מתהפך לקורא/ת עברית)", () => {
  ...
  it("פסיק שמפריד בין שני חלקי משפט עברי – לא כורכים את שני הצדדים יחד", () => {
    ...
    expect(cores("מסודר: x מתחת ל-x, y מתחת ל-y).")).toEqual([]);
  });
  it("מקף עברי צמוד לאות – חלק מהמילה, לא מינוס של הביטוי", () => {
    // "פתרי את מערכת המשוואות (מצאי x ו-y):" נקרא אצל נגה "…(מצאי x ו)y-:"
    expect(isolated("פתרי את מערכת המשוואות (מצאי x ו-y):")).toBe(false);
    expect(isolated("השיפוע – המספר שצמוד ל-x.")).toBe(false);
    expect(isolated("מסדרים: x מתחת ל-x, y מתחת ל-y, מספרים מתחת למספרים.")).toBe(false);
    // אבל מה שאחרי המקף כן ביטוי שלם – מבודדים אותו בלי המקף
    expect(cores("שימי לב ש-(a+b)(a−b) = a²−b². זה הכלל.")).toEqual(["(a+b)(a−b) = a²−b²."]);
  });
  it("סוגר שאין לו זוג בריצה שייך למשפט העברי", () => {
    // הסוגר של "(בצורה …)" שייך למשפט – בלי בידוד הביטוי ממילא יוצא LTR כמו שצריך
    expect(isolated("מצאי את משוואת הישר (בצורה y=mx+b): קודם השיפוע.")).toBe(false);
    // סוגריים מאוזנים בתוך הריצה – כן מבודדים
    expect(cores("הכלל: (a+b)(a−b) = a²−b². זה הכול.")).toEqual(["(a+b)(a−b) = a²−b²."]);
    expect(cores("סימנים הפוכים (+y ו−y) → מחברים.")).toEqual([]);
    expect(cores("**אותו סימן** (+y ו+y) → **מחסרים**.")).toEqual([]);
  });
});
