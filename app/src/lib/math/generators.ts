...
export function genLinearEq(level: number): Exercise {
  ...
  if (level === 1) {
    ...
    stages.push(
      S("מר גזען – להפריד", "נעלמים בצד אחד, מספרים בצד שני. מה שעובר צד – עובר דרך מראת הקסמים ומתהפך (+ ↔ −).", `העבירי את ${b >= 0 ? b : `(${b})`} לצד השני – הוא ${b >= 0 ? "פלוס, אז יהפוך למינוס" : "מינוס, אז יהפוך לפלוס"}.`),
      S("חילוק במקדם", `כמה שווה פרה אחת? יש ${Math.abs(a)} פרות. כפל עובר צד והופך לחילוק.`, `חלקי את שני הצדדים ב-${a}.`)
    );
    mapTags.families = 0;
    mapTags.divide = 1;
    mapTags.final = 2;
    steps.push(st(`${L(x.scale(a))}=${L(rhs.sub(Poly.const(b)))}`, 1, "מר גזען"), st(`x=${sol}`, 2, "חילוק במקדם"));
  } else if (level === 2) {
    ...
    stages.push(
      S("מר גזען + שונא שליליים", "נעלמים לצד אחד, מספרים לצד שני. איזה צד? הצד שבו האיקסים יצאו *חיוביים* – שונא שליליים יחיה!", "העבירי את האיקסים לצד שבו יש יותר איקסים, ואת המספרים לצד השני. מה שעובר – מתהפך."),
    ...
    stages.push(
      ...
      S("מר גזען + שונא שליליים", "נעלמים לצד אחד, מספרים לצד שני; האיקסים לצד שבו יצאו חיוביים.", "העבירי צדדים. אם האיקסים נעלמים משני הצדדים – בדקי מה נשאר: 0=0 → כל x; מספר≠מספר → אין x."),
    ...
    if (solutions === "all") steps.push(st(`0=0`, 3, "הכול התקזז – כל x!"));
      ...
      const coef = left ? la - ra : ra - la;
      const cst = left ? rb - lb : lb - rb;
      steps.push(st(left ? `${L(x.scale(coef))}=${cst}` : `${cst}=${L(x.scale(coef))}`, 3, "מר גזען"), st(`x=${sol}`, 4, "חילוק במקדם"));
    }
  }
  return {
    id: uid("lin"),
...
export function genLinearEqFrac(level: number): Exercise {
  ...
  const stages: StageInfo[] = [
    ...
    S("מר גזען + איחוד משפחות", "נעלמים בצד אחד, מספרים בצד שני, ואז חתולים עם חתולים.", "העבירי צדדים וחברי."),
    S("חילוק במקדם", "כפל עובר צד והופך לחילוק.", "חלקי במקדם של x."),
  ];
  const steps: Step[] = [];
  const traps: Trap[] = [];
  ...
  if (level === 1) {
    ...
    rhsStr = `${c2}`;
    void c;
    steps.push(st(`x+${a * b}=${a * c2}`.replace("+-", "-"), 1, "טרומפלדור: הכפלנו הכול ב-" + a), st(`x=${s2}`, 3, "מר גזען"));
    return {
      id: uid("linf"),
  ...
  if (level === 2) {
    ...
    steps.push(st(`${lcm / c}\\left(${L(p1)}\\right)=${lcm / f}\\left(${L(p2)}\\right)`, 1, `הכפלנו הכול ב-${lcm} (טרומפלדור)`), st(`${L(p1.scale(lcm / c))}=${L(p2.scale(lcm / f))}`, 2, "נשיקות"), st(`x=${s2}`, 4, "מר גזען וחילוק"));
  ...
  void lhsStr;
  void rhsStr;
  steps.push(st(`${mc}\\left(${L(p1)}\\right)+${mf}\\left(${L(p2)}\\right)=${L(x.scale(g * lcm))}`, 1, `הכפלנו הכול ב-${lcm}`), st(`${L(p1.scale(mc).add(p2.scale(mf)))}=${L(x.scale(g * lcm))}`, 2, "נשיקות ואיחוד משפחות"), st(`x=${s2}`, 4, "מר גזען וחילוק"));
  return {
    id: uid("linf"),
    typeId: "linear_eq_frac",
    topicId: "linear_eq_frac",
...
export function genSystem(level: number): Exercise {
  ...
  const stages: StageInfo[] = [
    ...
    S("פותרים נעלם אחד", "נשארה משוואה רגילה עם x – מר גזען וחילוק.", "פתרי את x."),
...
export function genShortMult(level: number): Exercise {
  ...
  if (level === 1) {
    ...
    plain = `(${P(sq)})^2`;
    traps.push({ plain: P(x.pow(2).add(Poly.const(a * a))), message: "חסר האיבר האמצעי. חזקה היא סנובית – היא לא מתחלקת על הפלוס. (a+b)² הוא לא a²+b²: קודם הופכים לכפל, ואז האצבע – כשמרימים אותה מקבלים 2·x·" + a + ".", mistake: "pow_sum" });
    ...
    prompt = `\\left(${L(sq)}\\right)^{2}`;
    plain = `(${P(sq)})^2`;
    traps.push({ plain: P(x.pow(2).scale(k).add(x.scale(2 * k * a)).add(Poly.const(a * a))), message: `(${k}${v})² זה ${k * k}${v}² – גם המקדם עולה בריבוע.`, mistake: "coef_sq" });
    traps.push({ plain: P(x.pow(2).scale(k * k).add(Poly.const(a * a))), message: "חסר האיבר האמצעי. חזקה היא סנובית – לא מתחלקת על הפלוס. קודם כפל, ואז האצבע.", mistake: "pow_sum" });
...
export function genTrinomial(level: number): Exercise {
  ...
  const listOf = (n: number) => {
    const m = Math.abs(n);
    const out: string[] = [];
    for (let i = 1; i * i <= m; i++) if (m % i === 0) out.push(`${i}·${m / i}`);
    return out.join(", ");
  };
  const gangHint = (p: number, q: number) => {
    const c = p * q,
      b = p + q;
    const q1 = c > 0 ? "האיבר החופשי חיובי – שניהם מאותה כנופיה (אותו סימן)." : "האיבר החופשי שלילי – כנופיות יריבות (סימן שונה).";
  ...
  if (level === 3 && rnd() < 0.35) {
    ...
    return {
      ...
      finalForm: "factored",
      stages: [
        S("התאומים ההפוכים", "שני איברים, שניהם ריבועים, ומינוס ביניהם = הפרש ריבועים. a²−b² = (a+b)(a−b): אותם שניים, סימן הפוך – האמצע מתחסל כמו כנופיות.", `מה בריבוע נותן ${L(x.scale(k).pow(2))}? ומה בריבוע נותן ${m * m}? כתבי (א+ב)(א−ב).`),
      ],
  ...
  if (level === 3 && rnd() < 0.5) {
    ...
    const q = rnz(-5, 5),
      sIn = rnz(-5, 5);
    const f1 = x.scale(pp).add(Poly.const(q)),
      f2 = x.scale(r).add(Poly.const(sIn));
    const total = f1.mul(f2);
    const A = pp * r,
      B = pp * sIn + q * r,
      C = q * sIn;
    // require no overall common factor (otherwise ועד בית רגיל קודם) and B ≠ 0
    if (gcd(gcd(Math.abs(A), Math.abs(B)), Math.abs(C)) !== 1 || B === 0) return genTrinomial(level);
    // split middle: B = m1 + m2, m1·m2 = A·C ; use m1 = pp*sIn, m2 = q*r
    const m1 = pp * sIn,
      m2 = q * r;
    const splitLatex = `${L(x.pow(2).scale(A))}${m1 >= 0 ? "+" : ""}${L(x.scale(m1))}${m2 >= 0 ? "+" : ""}${L(x.scale(m2))}${C >= 0 ? "+" : ""}${C}`;
    // group: pp*x*(r x + sIn) + q*(r x + sIn)
    const g1 = x.scale(pp),
      g2 = Poly.const(q);
    const groupLatex = `${L(g1)}\\left(${L(f2)}\\right)${q >= 0 ? "+" : ""}${q === 1 ? "" : q === -1 ? "-" : q}\\left(${L(f2)}\\right)`;
    void g2;
    return {
      id: uid("tri2"),
      typeId: "trinomial",
      ...
      kind: "expr",
      level,
      instruction: "פרקי לגורמים (המקדם של הריבוע אינו 1 – ועד בית פעמיים):",
      promptLatex: L(total),
      originalPlain: P(total),
      ...
      finalForm: "factored",
      stages: [
        S("ועד בית קודם? ואז קצה בקצה", `אין גורם משותף לכולם. אז: מכפילים קצה בקצה – ${A}·(${C}) = ${A * C}. עכשיו מחפשים זוג שמכפלתו ${A * C} וסכומו ${B}. ${gangHint(m1, m2)}`, `הזוג: ${m1} ו-${m2}. בדקי: ${m1}·(${m2})=${A * C}, ${m1}+(${m2})=${B}.`),
        S("פיצול האמצע", `מפצלים את האיבר האמצעי לשניים לפי הזוג. הביטוי לא השתנה – רק נכתב בארבעה איברים.`, `כתבי: ${splitLatex.replace(/\\\\/g, "")}`),
        S("ועד בית בכל חצי", "מוציאים גורם משותף מהזוג הראשון, ואז מהזוג השני. אם עבדת נכון – בשני החצאים נשאר אותו גוש בסוגריים.", `כתבי: ${groupLatex.replace(/\\\\left|\\\\right/g, "").replace(/\\\\/g, "")}`),
        S("הגוש המשותף החוצה", "הסוגר כולו הוא איבר אחד – גוש. הוועד מוציא אותו החוצה, ומה שנשאר נכנס לסוגריים השניים.", `(${L(f2).replace(/\\\\/g, "")})(${L(f1).replace(/\\\\/g, "")}) – ובדיקה: 4 נשיקות חזרה.`),
      ],
      steps: [st(splitLatex, 2, "פיצול האמצע"), st(groupLatex, 3, "ועד בית בכל חצי"), st(`\\left(${L(f2)}\\right)\\left(${L(f1)}\\right)`, 4, "הגוש המשותף")],
      stageOf: (info) => {
        if (!info.node) return 0;
        const pc = parenCount(info.node);
        if (pc >= 2) return 3;
        if (nodeCount(info.node) > nodeCount(parseExpr(P(total))!)) return 2;
        return 0;
      },
      traps: [],
    };
  ...
  return {
    ...
    finalForm: "factored",
    stages: [
      S("זוג שמסתדר", `מחפשים שני מספרים ש*במכפלה* נותנים ${p * q} ו*בסכום* נותנים ${p + q}. ${gangHint(p, q)} חוק הרשימה: הזוגות של ${Math.abs(p * q)} הם ${listOf(p * q)} – עוברים פעם אחת.`, `הזוג הוא ${p} ו-${q}. בדקי: ${p}·(${q})=${p * q}, ${p}+(${q})=${p + q}.`),
      S("כתיבת הסוגריים", `כל אחד מבני הזוג נכנס לסוגריים משלו עם ${v}. בדיקה: פתחי חזרה (4 נשיקות) ותקבלי את המקור.`, `כתבי (${v}${p >= 0 ? "+" : ""}${p})(${v}${q >= 0 ? "+" : ""}${q}).`),
    ],
    steps: [st(`\\left(${L(f1)}\\right)\\left(${L(f2)}\\right)`, 2, "הזוג שמסתדר")],
    stageOf: () => 0,
    traps: [
      { plain: `(${P(x.sub(Poly.const(p)))})*(${P(x.sub(Poly.const(q)))})`, message: "הזוג נכון – הסימנים הפוכים. שאלה 2: מי המנהיג? האיבר האמצעי קובע.", mistake: "sign" },
    ],
  };
}
...
export function genQuadraticEq(level: number): Exercise {
  const v = "x";
  const x = Poly.v(v);
  let lhs: Poly, roots: number[] | "none", stages: StageInfo[], steps: Step[], promptLatex: string;
  let instruction = "פתרי את המשוואה (מצאי את כל הפתרונות):";
  ...
  if (level === 1) {
    ...
    promptLatex = `${L(lhs)}=0`;
    stages = [
      S("ועד בית – לא חילוק ב-x!", "שני איברים, לשניהם יש x. אף פעם לא מחלקים ב-x לפני שבודקים אם הוא אפס – מוציאים אותו כגורם משותף: הוועד הקומוניסטי.", `כתבי x(x${b >= 0 ? "+" : ""}${b})=0.`),
      S(ZERO, zeroHint, `x=0 או x${b >= 0 ? "+" : ""}${b}=0.`),
    ];
    steps = [st(`x\\left(${L(x.add(Poly.const(b)))}\\right)=0`, 1, "ועד בית"), st(`x=0,\\ x=${-b}`, 2, ZERO)];
    ...
    steps = [st(`\\left(x+${m}\\right)\\left(x-${m}\\right)=0`, 1, "תאומים הפוכים"), st(`x=${m},\\ x=${-m}`, 2, ZERO)];
  } else if (level === 2 || (level === 3 && roll < 0.35)) {
    const p = rnz(-9, 9),
      q = rnz(-9, 9);
    ...
    stages = [
      ...
      S("זוג שמסתדר – חוק הרשימה", `מכפלה ${p * q}, סכום ${p + q}. שאלה 1: החופשי ${p * q > 0 ? "חיובי – אותה כנופיה" : "שלילי – יריבים"}. שאלה 2: האמצעי קובע. רשימת הזוגות של ${Math.abs(p * q)} – פעם אחת, ואם נגמרה: נוסחה.`, `הזוג: ${p} ו-${q}. כתבי (x${p >= 0 ? "+" : ""}${p})(x${q >= 0 ? "+" : ""}${q})=0.`),
      S(ZERO, zeroHint, `x=${-p} או x=${-q}.`),
    ];
    steps = [];
    if (shift) steps.push(st(`${L(lhs)}=0`, 1, "הכול לצד אחד"));
    steps.push(st(`\\left(${L(x.add(Poly.const(p)))}\\right)\\left(${L(x.add(Poly.const(q)))}\\right)=0`, 2, "זוג שמסתדר"), st(`x=${-p},\\ x=${-q}`, 3, ZERO));
  } else if (level === 3 && roll < 0.65) {
    // נוסחת השורשים: (px+q)(x+s)=0 → a≠1
    const pp = pick([2, 2, 3, 4, 5]);
    const q = rnz(-7, 7),
      sIn = rnz(-7, 7);
    if (gcd(Math.abs(q), pp) !== 1) return genQuadraticEq(level);
    lhs = x.scale(pp).add(Poly.const(q)).mul(x.add(Poly.const(sIn)));
    const A = pp,
      B = pp * sIn + q,
      C = q * sIn;
    if (B === 0 || C === 0) return genQuadraticEq(level);
    abc = [A, B, C];
    roots = [-q / pp, -sIn];
    promptLatex = `${L(lhs)}=0`;
    ...
    stages = [
      S("שלוש שורות: a, b, c", `המקדם של x² אינו 1 – ואין גורם משותף. או ועד בית פעמיים, או נוסחה. הנוסחה בנוסחאון – לא צריך לזכור. קודם רושמים בצד, עם הסימנים: a=${A}, b=${B}, c=${C}.`, `a=${A}, b=${B}, c=${C}. עכשיו מה שמתחת לשורש: b²−4ac = ${B}²−4·${A}·(${C}) = ${disc}.`),
      S("מתחת לשורש", `b²−4ac = ${disc}. ${disc >= 0 ? "חיובי – יש פתרונות. שורש " + disc + " = " + sq + "." : "שלילי – אין פתרון."}`, `x = (${-B} ± ${sq}) / ${2 * A}.`),
      S("שני הפתרונות", "פלוס בנפרד, מינוס בנפרד. שבר? משאירים כשבר מצומצם או כעשרוני – שניהם מתקבלים.", `x=${fracLatex(frac(-B + sq, 2 * A))}, x=${fracLatex(frac(-B - sq, 2 * A))}.`),
    ];
    steps = [
      st(`a=${A},\\ b=${B},\\ c=${C}`, 1, "שלוש שורות בצד"),
      st(`x_{1,2}=\\frac{${-B}\\pm\\sqrt{${disc}}}{${2 * A}}=\\frac{${-B}\\pm${sq}}{${2 * A}}`, 2, "הנוסחה"),
      st(`x=${fracLatex(frac(-B + sq, 2 * A))},\\ x=${fracLatex(frac(-B - sq, 2 * A))}`, 3, "שני הפתרונות"),
    ];
    instruction = "פתרי את המשוואה (הנוסחה בנוסחאון 📄):";
  } else {
    // דיסקרימיננטה שלילית – אין פתרון
    const b = rnz(-6, 6);
    const c = rint(Math.floor((b * b) / 4) + 1, Math.floor((b * b) / 4) + 6);
    lhs = x.pow(2).add(x.scale(b)).add(Poly.const(c));
    roots = "none";
    abc = [1, b, c];
    promptLatex = `${L(lhs)}=0`;
    const disc = b * b - 4 * c;
    stages = [
      S("חוק הרשימה", `מכפלה ${c}, סכום ${b}. עוברים על הזוגות של ${c} פעם אחת. לא מצאת? אין זוג – מזמינים את הנוסחה.`, `a=1, b=${b}, c=${c}.`),
      S("מתחת לשורש", `b²−4ac = ${b}²−4·1·${c} = ${disc}. שלילי.`, "אם מה שמתחת לשורש שלילי – אין פתרון. עוצרים, לא ממשיכים. לחצי 'אין פתרון'."),
    ];
    steps = [st(`a=1,\\ b=${b},\\ c=${c}`, 1, "שלוש שורות"), st(`b^{2}-4ac=${disc}<0`, 2, "מתחת לשורש שלילי – אין פתרון")];
    instruction = "פתרי את המשוואה (ייתכן שאין פתרון):";
  }
  return {
    id: uid("quad"),
    typeId: "quadratic_eq",
    ...
    instruction,
    promptLatex,
    finalLatex: roots === "none" ? "\\text{אין פתרון}" : roots.map((r) => `x=${Number.isInteger(r) ? r : fracLatex(frac(Math.round(r * 60), 60))}`).join(",\\ "),
    variable: v,
    solutions: roots,
    abc,
    stages,
...
export function genOrderOps(level: number): Exercise {
  ...
  const traps: Trap[] = [];
  const stages: StageInfo[] = [];
  const PAIR = "הפעולות באות בזוגות: חזקות עם שורשים, כפל עם חילוק, חיבור עם חיסור. בתוך זוג אין קדימות – משמאל לימין, לפי מי שהגיע ראשון.";
  if (level === 1) {
    // a·b − c + d   (pair trap: c+d first)
    const ab = a * b + 5;
    const cc = rint(2, ab - 1),
      dd = rint(1, 9);
    latex = `${a}\\cdot ${b + 1}-${cc}+${dd}`;
    plain = `${a}*${b + 1}-${cc}+${dd}`;
    value = a * (b + 1) - cc + dd;
    traps.push({ plain: `${a * (b + 1) - (cc + dd)}`, message: `חיבור וחיסור הם זוג – אותה דרגה. אין "קודם החיבור". משמאל לימין: קודם ${a * (b + 1)}−${cc}, ואז +${dd}.`, mistake: "pair_order" });
    stages.push(S("סנובים קודם, ובזוג – משמאל לימין", `הכפל סנוב יותר מהחיבור – הוא ראשון. אחר כך חיבור וחיסור, שהם זוג: ${PAIR}`, `${a}·${b + 1}=${a * (b + 1)}. ואז משמאל לימין: ${a * (b + 1)}−${cc}=${a * (b + 1) - cc}, ועוד ${dd}.`));
  } else if (level === 2) {
    // p − q·(r−s)² : t + u
    const r = rint(4, 9),
      sS = rint(1, r - 2);
    const diff = r - sS;
    const q = rint(2, 4);
    const tOpts = [1, 2, 3, 4, 6, 9].filter((k) => k > 1 && (q * diff * diff) % k === 0);
    const t = tOpts.length ? pick(tOpts) : 1;
    const pP = rint(10, 30),
      u = rint(1, 9);
    latex = `${pP}-${q}\\cdot\\left(${r}-${sS}\\right)^{2}:${t}+${u}`;
    plain = `${pP}-${q}*(${r}-${sS})^2/${t}+${u}`;
    const mid = (q * diff * diff) / t;
    value = pP - mid + u;
    traps.push({ plain: `${pP - (mid + u)}`, message: `חיבור וחיסור הם זוג. משמאל לימין: קודם ${pP}−${mid}, ורק אז +${u}. לא ${mid}+${u} קודם.`, mistake: "pair_order" });
    traps.push({ plain: `${pP - (q * (r * r - sS * sS)) / t + u}`, message: "(r−s)² זה לא r²−s². חזקה היא סנובית – קודם מסדרים בתוך הסוגריים, ואז מעלים בריבוע.", mistake: "pow_sum" });
    stages.push(S("סוגריים → חזקה → כפל וחילוק → חיבור וחיסור", `הסוגריים הכי סנוביות: ${r}−${sS}=${diff}. אז החזקה: ${diff}²=${diff * diff}. ${PAIR}`, `${q}·${diff * diff}=${q * diff * diff}, חלקי ${t} = ${mid}. ואז משמאל לימין: ${pP}−${mid}+${u}.`));
  } else {
    // A : B · C − D·(−E)²
    const B = rint(2, 6),
      C = rint(2, 5),
      ...
      D = rint(2, 5),
      E = rint(2, 4);
    latex = `${A}:${B}\\cdot ${C}-${D}\\cdot\\left(-${E}\\right)^{2}`;
    plain = `${A}/${B}*${C}-${D}*(-${E})^2`;
    value = (A / B) * C - D * E * E;
    traps.push({ plain: `${A / (B * C) - D * E * E}`, message: `כפל וחילוק הם זוג – אותה דרגה, משמאל לימין. קודם ${A}:${B}=${A / B}, ואז ·${C}. לא ${B}·${C} קודם.`, mistake: "pair_order" });
    traps.push({ plain: `${(A / B) * C + D * E * E}`, message: `מלכודת המינוס: (−${E})² – המינוס בסוגריים מתחתן עם החזקה הזוגית ונעלם: ${E * E}. אבל המינוס שלפני ${D} נשאר בחוץ: −${D}·${E * E}.`, mistake: "sign" });
    stages.push(S("זוגות – משמאל לימין", `${PAIR} (−${E})²=${E * E} כי המינוס בפנים מתחתן עם החזקה הזוגית.`, `${A}:${B}=${A / B}, ·${C}=${(A / B) * C}. ${D}·${E * E}=${D * E * E}. ואז ${(A / B) * C}−${D * E * E}.`));
  }
  return {
    id: uid("ops"),
    typeId: "order_ops",
    ...
    kind: "expr",
    level,
    instruction: "חשבי לפי סדר פעולות חשבון (אפשר שלב-שלב):",
    promptLatex: latex,
    originalPlain: plain,
    finalPlain: `${value}`,
    finalLatex: `${value}`,
    finalForm: "any",
    stages,
    steps: [st(`${value}`, 1, "סנובים קודם, ובזוג משמאל לימין")],
    stageOf: () => 0,
    traps,
  };
}
...
/* =====================================================================
   14. תחום הצבה + צמצום שברים אלגבריים – השומר בכניסה
   ===================================================================== */
export function genAlgFrac(level: number): Exercise {
  const v = "x";
  const x = Poly.v(v);
  let num: Poly, den: Poly, numF: string, denF: string, finalPlain: string, finalLatex: string, excluded: number[], factorNote: string;
  const traps: Trap[] = [];
  if (level === 1) {
    // (k x + k a) / (x² + a x) = k / x  ,  excluded 0, -a
    const k = pick([2, 3, 4, 5, 6]),
      a = rnz(-9, 9);
    num = x.add(Poly.const(a)).scale(k);
    den = x.mul(x.add(Poly.const(a)));
    numF = `${k}\\left(${L(x.add(Poly.const(a)))}\\right)`;
    denF = `x\\left(${L(x.add(Poly.const(a)))}\\right)`;
    finalPlain = `${k}/x`;
    finalLatex = `\\frac{${k}}{x}`;
    excluded = [0, -a];
    factorNote = `למעלה ועד בית ${k}, למטה ועד בית x. גוש (x${a >= 0 ? "+" : ""}${a}) למעלה ולמטה – פלאש-דאון.`;
    ...
    if (rnd() < 0.5) {
      ...
      numF = `\\left(x+${m}\\right)\\left(x-${m}\\right)`;
      denF = `x\\left(x+${m}\\right)`;
      finalPlain = `(x-${m})/x`;
      finalLatex = `\\frac{x-${m}}{x}`;
      excluded = [0, -m];
      factorNote = `למעלה תאומים הפוכים, למטה ועד בית x. הגוש (x+${m}) משותף – פלאש-דאון.`;
      traps.push({ plain: `(-${m * m})/(${m}*x)`, message: "צמצמת x² עם x² כאילו הם גושים – אבל הם חלק מחיבור/חיסור. אסור. קודם מפרקים לגושים.", mistake: "cancel" });
    } else {
      const a = rnz(-7, 7),
        b = rnz(-7, 7),
        c = rnz(-7, 7);
      if (a === b || a === c || b === c) return genAlgFrac(level);
      num = x.add(Poly.const(a)).mul(x.add(Poly.const(b)));
      den = x.add(Poly.const(a)).mul(x.add(Poly.const(c)));
      numF = `\\left(${L(x.add(Poly.const(a)))}\\right)\\left(${L(x.add(Poly.const(b)))}\\right)`;
      denF = `\\left(${L(x.add(Poly.const(a)))}\\right)\\left(${L(x.add(Poly.const(c)))}\\right)`;
      finalPlain = `(${P(x.add(Poly.const(b)))})/(${P(x.add(Poly.const(c)))})`;
      finalLatex = `\\frac{${L(x.add(Poly.const(b)))}}{${L(x.add(Poly.const(c)))}}`;
      excluded = [-a, -c];
      factorNote = `למעלה ולמטה הזוג שמסתדר. הגוש (${L(x.add(Poly.const(a)))}) משותף – פלאש-דאון.`;
    }
  } else {
    // (x+a)(x+b) / (x² − a²) → (x+b)/(x−a)   or  k(x+a)(x+b) / (x²−a²)·? keep simple with sign flip: (a−x) variant
    const a = rint(2, 8),
      b = rnz(-8, 8);
    ...
    denF = flip ? `\\left(${a}+x\\right)\\left(${a}-x\\right)` : `\\left(x+${a}\\right)\\left(x-${a}\\right)`;
    finalPlain = flip ? `-(${P(x.add(Poly.const(b)))})/(x-${a})` : `(${P(x.add(Poly.const(b)))})/(x-${a})`;
    finalLatex = flip ? `-\\frac{${L(x.add(Poly.const(b)))}}{x-${a}}` : `\\frac{${L(x.add(Poly.const(b)))}}{x-${a}}`;
    excluded = [a, -a];
    factorNote = flip ? `למטה תאומים הפוכים: (${a}+x)(${a}−x). ו-(${a}−x) מול (x−${a}) זה חיסור הפוך – מינוס 1.` : `למעלה הזוג שמסתדר, למטה תאומים הפוכים. הגוש (x+${a}) משותף.`;
    if (flip) traps.push({ plain: `(${P(x.add(Poly.const(b)))})/(x-${a})`, message: "חסר המינוס של חיסור הפוך: (a−x) חלקי (x−a) זה −1, לא 1.", mistake: "sign" });
  }
